...
export function BottomNav() {
  ...
  const { pageActuelle, setPageActuelle, notificationsNonLues, messagesNonLus } =
    useAuthStore();
  const pathname = usePathname();

  const items = [
    { label: "Accueil", icon: <Home size={20} />, href: "/accueil", page: "Accueil" },
    { label: "Dashboard", icon: <LayoutDashboard size={20} />, href: "/dashboard", page: "DashBoard" },
    { label: "Fiches", icon: <FileText size={20} />, href: "/feuilles-heures", page: "Fiches heures" },
    {
      label: "Messages",
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-secondary-bg border-t border-alternate shadow-lg">
      <div className="flex items-center justify-around h-16 px-2">
        {items.map((item) => {
          const isActive =
            pageActuelle === item.page || pathname === item.href;
